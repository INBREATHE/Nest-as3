...
import nest.packer.RectanglePacker;

public class RasterizerService extends EventDispatcher
{
	static private const FILE_EXTENSION_PNG:String = ".png";
	static private const FILE_EXTENSION_XML:String = ".xml";

	static private const BTN_DOWN:String = "_down";
	static private const BTN_NAME_DOWN:Array = ["", BTN_DOWN];
	...
	static private const LAYOUT_BACKGROUND_NAME:String = "background";

	static private var _instance:RasterizerService;
	static public function getInstance():RasterizerService {
		return _instance;
	}
	...
	public function addElementToRaster(
		...
		if(isButton) {
			...
			RegisterRasterItem("", RasterTypes.BUTTON, storeName, SimpleButton(element).upState, position)
			RegisterRasterItem("", RasterTypes.BUTTON, BTN_NAME_DOWN.join(""), SimpleButton(element).downState, position)
		} else {
	...
	private function ProcessAndSave():void {
		...
		var processPackage:Function = function(atlasIndex:uint):void
		{
			...
			file = File.cacheDirectory.resolvePath(String(fileAtlasName + FILE_EXTENSION_PNG));
	...
	private function HandlerReadXMLFileComplete(e:Event):void {
		...
		var loadTextures:Function = function ():void
		{
			if (texturesStack.length) {
				...
				fileAtlas = File.cacheDirectory.resolvePath(fileName + FILE_EXTENSION_PNG);
				if(fileAtlas.exists) {
